...
왼쪽 **전체 맵**을 누르면 연결한 프로젝트와 자동 감지된 작업 폴더가 각각 작은 오피스로 표시됩니다. 프로젝트를 전환하지 않고 여러 곳의 활동을 함께 볼 수 있습니다.

![전체 프로젝트 맵](docs/images/project-map-desktop.png)

- 프로젝트별 활동 인원, 응답이 필요한 앱 작업, 외부 세션 수를 실시간으로 표시합니다. 같은 이름의 폴더는 경로로 구분합니다.
- 활동 중인 캐릭터는 책상에, 완료·대기한 캐릭터는 휴식 공간에 표시합니다. 상태를 오래 확인하지 못한 세션은 별도로 안내합니다.
- 각 공간에는 응답이 필요한 동료와 활동 중인 동료부터 최대 4명을 표시합니다. **더 보기**로 나머지 동료를 확인합니다.
- 캐릭터를 누르면 해당 프로젝트와 세션을 바로 선택합니다. 외부 세션의 작업 내역·대화 또는 앱 작업의 승인 화면으로 이어집니다. 프로젝트 이름을 누르면 해당 오피스가 열립니다.
...
- 전체 맵이나 오피스 아래 **퇴근한 동료**를 펼쳐 **다시 출근**할 수 있습니다. 퇴근 상태는 서버 재시작 후에도 유지됩니다. 원본 로그가 관측 범위를 벗어났다면 원래 CLI에서 세션을 열어 다시 감지시켜 주세요.
