...
export function phasePrompt(run:Run,provider:Provider,role:'implementer'|'reviewer',previous:string,changes:Change[],review?:Review){
 return [
  ...
  role==='reviewer'?'변경 파일을 직접 읽고 정확성과 요구사항 충족 여부를 검토하세요. 테스트는 가능한 범위에서 실행하되 소스 파일은 수정하지 마세요. verdict는 pass / changes_requested / inconclusive 중 하나, summary는 검증 범위 설명, findings는 {path,message,severity:error|warning} 배열로 응답하세요. 검사하지 못한 중요 항목이 있으면 inconclusive로 표시하세요.':'필요한 파일을 수정하고 가능한 테스트를 실행하세요. 마지막에 변경 내용과 실제 실행한 검증, 남은 문제를 보고하세요.'
 ].filter(Boolean).join('\n\n');
}
