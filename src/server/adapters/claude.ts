  ...
  return {
    async probe() {
      try {
        ...
        const r = await exec('claude', ['auth', 'status'], { timeout: 5000 });
        ...
        return {
          ...
          detail: s.loggedIn
            ? 'Claude Code 로그인 연결'
            : '터미널에서 claude auth login을 실행해 주세요.',
        };
      } catch {
        return {
          installed: false,
          authenticated: null,
          detail: 'Claude Code 설치 및 로그인을 확인해 주세요.',
        };
      }
    },
          ...
          options: {
              ...
              PreToolUse: [
                {
                  hooks: [
                    async (h) => {
                      ...
                      const args = h.tool_input as Record<string, unknown>;
                      ...
                      if (tool === 'Bash')
                        return {
                          hookSpecificOutput: {
                            ...
                            permissionDecisionReason: '실행할 명령을 확인해 주세요.',
                          },
                        };
                      return {};
                    },
            ...
            canUseTool: async (tool, args) => {
              if (input.signal.aborted)
                return { behavior: 'deny', message: '작업이 중단되었습니다.' };
              const question = tool === 'AskUserQuestion';
              const answer = await interact({
                runId: input.runId,
                agentId: 'claude',
                kind: question ? 'question' : 'approval',
                title: question ? 'Claude의 질문' : String(args.command ?? `${tool} 작업 승인`),
                details: { tool, ...args },
              });
              if (question && 'answers' in answer)
                return {
                  behavior: 'allow',
                  updatedInput: {
                    ...args,
              ...
              return 'decision' in answer && answer.decision === 'approve'
                ? { behavior: 'allow', updatedInput: args }
                : { behavior: 'deny', message: '사용자가 작업을 거절했습니다.' };
            },
          },
        });
        active = stream;
        for await (const m of stream) {
          ...
          if (m.type === 'system' && m.subtype === 'init')
            emit({
              ...
              agentId: 'claude',
              type: 'agent.session',
              payload: { sessionId: m.session_id, model: m.model },
            });
          if (
            m.type === 'stream_event' &&
          ...
          if (m.type === 'result') {
            ...
            return {
              ...
              error: parsed && !parsed.success ? '검토 응답 형식을 확인할 수 없습니다.' : undefined,
            };
          }
        }
        return {
